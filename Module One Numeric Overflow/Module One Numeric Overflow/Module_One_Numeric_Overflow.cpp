...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i) {
        // Prevent overflow for signed types
        if (std::numeric_limits<T>::is_signed) {
            if (increment > 0 && result > std::numeric_limits<T>::max() - increment) {
                std::cout << "Overflow detected during addition!\n";
                return 0;  // Return a failure indicator
            }
        } else {
            // Prevent overflow for unsigned types
    ...
    return result; //Successful computation
}
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i) {
        // For signed types:
        if (std::numeric_limits<T>::is_signed) {
            // Checking for underflow when decrementing
            if (decrement > 0 && result < std::numeric_limits<T>::min() + decrement) {
                std::cout << "Underflow detected during subtraction!\n";
