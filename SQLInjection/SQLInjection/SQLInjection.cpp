...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Regular expression to detect "OR value=value;" patterns
  // Matches cases like "OR 1=1", "OR 'hack'='hack'", "OR 'a'='a';"
  std::regex injection_pattern(R"(or\s+(?:['\"]?[a-zA-Z0-9]+['\"]?\s*=\s*['\"]?[a-zA-Z0-9]+['\"]?|[0-9]+\s*=\s*[0-9]+)\s*;?)", std::regex::icase);

  // Check if the query matches the suspicious pattern
  if (std::regex_search(sql, injection_pattern))
  {
    std::cout << "Potential SQL Injection detected and prevented! Query: " << sql << std::endl;
