...
std::string read_file(const std::string& filename)
{
  ...
  if (infile.is_open())
  {
    ...
    ss << infile.rdbuf(); // Read the entire file into a string
    file_text = ss.str();

    // Normalize line endings (remove carriage returns)
    file_text.erase(std::remove(file_text.begin(), file_text.end(), '\r'), file_text.end());
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  if (outfile.is_open())
  {
    ...
    // Write current date as timestamp (yyyy-mm-dd)
    std::time_t t = std::time(nullptr);
    std::tm now;
    if (localtime_s(&now, &t) == 0) // Check for errors
    {
